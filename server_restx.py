...
try:
    from src.scraper import scrape_tokopedia as scraper_func
...
scrape_request = api.model(
    "ScrapeRequest",
    {
        "query": fields.String(required=True, description="Search query"),
        "num_products": fields.Integer(
            default=10, description="Number of products to scrape"
        ),
    },
)

scrape_response = api.model(
    "ScrapeResponse",
    {
...
class ScrapeResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            # Prevent injection attacks - only allow alphanumeric and basic punctuation
            import re

            if not re.match(r"^[\w\s\-\_\.\,\+\&\#]+$", query):
                api.abort(400, "Query contains invalid characters")

            num_products = data.get("num_products", 10)
            if (
                not isinstance(num_products, int)
                or num_products < 1
                or num_products > 50
            ):
